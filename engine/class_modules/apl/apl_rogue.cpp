...
void outlaw( player_t* p )
{
  ...
  default_->add_action( "variable,name=halfway_through_gcd,value=gcd.remains<=action.sinister_strike.gcd%2", "Variable to use certain off-gcd actions at least halfway through the gcd instead of instantly" );
  default_->add_action( "call_action_list,name=cds" );
